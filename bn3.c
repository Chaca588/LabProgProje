...
int defesaqm(player_infoq* defensor, int smapa, int fd){//funcao para os marcar tiros da multi-shell Qtree
    int x, y;
    sleep(1);
...
int main(int argc, char const *argv[]){
  ...
  while(val != 7){
    defesaqm(player2, smapa, fd);//player 2 defende
    if(player2->hptotal == 0){
        printf("Player 1 ganha!");
        goto fim;//salta para o fim do jogo
    }
    val = ataqueqm(player2, smapa, fd);//player 2 ataca
    print_val(val);
  ...
  freeallq(player2, smapa);//Libertação da memória alocada anteriormente
  close(fd);//fecha o fd
  exit(0);
}
